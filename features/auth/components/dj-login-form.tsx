...
export default function DJLoginForm() {
  ...
  const canSubmitOwner = Boolean(email.trim() && password.trim());
  const canSubmitHelper = Boolean(helperCode.trim() && /^\d{6}$/.test(helperPin));
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isHelper) {
      if (!helperCode.trim() || !/^\d{6}$/.test(helperPin)) {
        setError("Please enter the event code and a 6-digit helper PIN.");
        return;
      }
      setSubmitting(true);
      try {
        const res = await fetch("/api/dj/helper-login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ code: helperCode, helperPin }),
        });
        ...
        show("Helper access granted — redirecting to the queue…");
        router.push("/helper-queue");
        router.refresh();
      } catch {
  ...
  return (
    <>
      ...
      <form className="mb-5 flex flex-col gap-3.5" onSubmit={handleSubmit} noValidate>
          ...
          <>
            ...
            <Field label="Helper PIN">
              <Input
                ...
                value={helperPin}
                onChange={(e) => setHelperPin(e.target.value.replace(/\D/g, ""))}
              />
            </Field>

            <p className="text-center text-[11px] leading-[1.6] text-muted">
              Ask the event owner for the helper PIN — no account needed.
            </p>
          </>
        ) : (
          <>
        ...
        <button
          type="submit"
          disabled={submitting || (isHelper ? !canSubmitHelper : !canSubmitOwner)}
